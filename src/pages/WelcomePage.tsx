    ...
    <div className="flex-1 h-full flex flex-col items-center justify-center bg-[var(--background)] p-8 overflow-y-auto">
      ...
      {dialog && <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/65 p-6">
        <form onSubmit={(event) => { event.preventDefault(); void submit(); }} className="w-full max-w-lg rounded-xl border border-slate-800 bg-[#191919] p-6 shadow-2xl">
          ...
          {dialog === 'clone' && <label className="mt-5 block text-sm text-gray-300">Repository URL<input required value={repositoryUrl} onChange={(event) => setRepositoryUrl(event.target.value)} placeholder="https://github.com/owner/repository.git" className="mt-1 h-10 w-full rounded border border-slate-800 bg-slate-950 px-3 outline-none focus:border-blue-500" /></label>}
          <label className="mt-4 block text-sm text-gray-300">Project folder name<input required value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="my-project" className="mt-1 h-10 w-full rounded border border-slate-800 bg-slate-950 px-3 outline-none focus:border-blue-500" /></label>
          {dialog === 'project' && <label className="mt-4 block text-sm text-gray-300">Starter template<select value={template} onChange={(event) => setTemplate(event.target.value)} className="mt-1 h-10 w-full rounded border border-slate-800 bg-slate-950 px-3 outline-none focus:border-blue-500"><option value="web">Web app (Vite)</option><option value="python">Python</option><option value="html">HTML, CSS and JavaScript</option></select></label>}
          <div className="mt-4"><p className="text-sm text-gray-300">Location</p><div className="mt-1 flex gap-2"><input required value={parentDirectory} readOnly placeholder="Choose a parent folder" className="h-10 min-w-0 flex-1 rounded border border-slate-800 bg-slate-950 px-3 text-sm text-gray-400 outline-none" /><button type="button" onClick={() => void chooseLocation()} className="rounded bg-slate-900 px-3 text-sm hover:bg-[#363636]">Browse</button></div></div>
          {error && <p className="mt-3 text-sm text-red-300">{error}</p>}
          <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setDialog(null)} className="rounded px-3 py-2 text-sm text-gray-300 hover:bg-slate-900">Cancel</button><button disabled={isWorking || !parentDirectory || !projectName.trim() || (dialog === 'clone' && !repositoryUrl.trim())} className="rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50">{isWorking ? 'Working…' : dialog === 'project' ? 'Create Project' : 'Clone Repository'}</button></div>
        </form>
      </div>}
    </div>
  );
