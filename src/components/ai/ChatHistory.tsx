        ...
        messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className={`max-w-[85%] text-sm rounded-lg p-3 ${
              msg.role === 'user' ? 'glass-panel text-gray-200' : 'text-gray-300'
            }`}>
              {msg.role === 'user' ? (
                    ...
                    code({node: _node, inline, className, children, ...props}: any) {
                      ...
                      return !inline && match ? (
                        <div className="relative group rounded bg-black my-2 overflow-hidden border border-[var(--border-color)]">
                          <div className="flex justify-between items-center px-3 py-1 glass-panel text-xs text-gray-400 border-b border-[var(--border-color)]">
                            <span>{match[1]}</span>
